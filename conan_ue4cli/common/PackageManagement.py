...
class PackageManagement(object):
	...
	def install(packageDir, channel, profile, args=[]):
		...
		'''
		return Utility.run(['conan', 'create', '.', '--user=adamrehn', '--channel=' + channel, '-pr:a', profile] + args, cwd=packageDir)
	...
	def generateWrapper(libName, template, delegates, packageDir, channel, profile):
		...
		PackageManagement.install(packageDir, channel, profile, ['env', 'conanbuild'])
	...
	def removeBasePackages():
		...
		'''
		Utility.run(['conan', 'remove', '--confirm', '*@adamrehn/profile'])
