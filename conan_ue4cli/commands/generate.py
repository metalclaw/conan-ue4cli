...
def generate(manager, argv):
	...
	with tempfile.TemporaryDirectory() as tempDir:
		...
		Utility.run(['conan', 'profile', 'show', '-s="env.UNREAL_ENGINE_VERSION={}"'.format(channel), '--profile:build={}'.format(profile), '--profile:host={}'.format(profile)])
		...
		if platform.system() == 'Linux':
			...
			Utility.run(['conan', 'profile', 'show', '-s="settings.compiler.libcxx=libc++"', '--profile:build={}'.format(profile), '--profile:host={}'.format(profile)])
